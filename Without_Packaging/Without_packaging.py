...
bin_loc_base = 'C:/Blender/Bins_W_O_P'
obj_loc_base = 'C:/Blender/Parts_W_O_P'
img_loc_base = 'C:/Blender/Images_W_O_P/Part'
num_of_objs = 200
Header = ['X','Y','Z','ID']
...
#directory labels
basestring = 4 #signifies length of folder number
...
def scenario(bin_loc, obj_loc, img_loc, Header, scene, num_of_objs, basestring, cyclenumber):
    ...
    for obj_cnt in range(num_of_objs):
        bpy.ops.import_mesh.stl(filepath = obj_loc)
        bpy.context.object.scale[0] = 0.03
        bpy.context.object.scale[1] = 0.03
        bpy.context.object.scale[2] = 0.03
        ...
        bpy.context.object.rigid_body.mass = 0.5
        bpy.context.object.rigid_body.restitution = 0.5
        bpy.context.object.location[0] = random.uniform(-3,3)
        bpy.context.object.location[1] = random.uniform(-5,5)
    ...
    cycle = str(cyclenumber).zfill(basestring)
    dir_name = img_loc + '/' + cycle
    if not exists(dir_name):
        mkdir(dir_name)
    else:
        rmtree(dir_name)
        print('Deleted the existing directory ' + str(cycle))
    ...
    DepthMap(scene)
    scene.render.filepath = depth_dir_name + '/ID' + str(pass_index_value)
    bpy.ops.render.render(write_still=True)
    
    for ID in range(1,pass_index_value):
            SubMasks(ID, depth_dir_name,'/ID')
                
    
    #rendered image for segmentation mask
    segment_dir_name = dir_name + '/SegmentationMasks'
    SegmentationMask(scene)
    scene.render.filepath = segment_dir_name +'/ID' + str(pass_index_value)
    bpy.ops.render.render(write_still=True)
    
    for ID in range(1,pass_index_value):
            SubMasks(ID, segment_dir_name,'/ID')
...
for part_index in range(len(parts)):
    ...
    img_loc = img_loc_base + str(part_index)
    cyclenumber = 0   
    part_name = parts[part_index][0 : 2]
    ...
    for bin_index in range(len(bins)):    
        ...
        scenario(bin_loc, obj_loc, img_loc, Header, scene, num_of_objs, basestring, cyclenumber)
        cyclenumber += 1
        pass_index_value = 1