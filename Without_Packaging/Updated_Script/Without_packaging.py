...
#without packaging working code. Old code has issues
...
def scenario(bin_loc, obj_loc, img_loc, Header, scene, num_of_objs, basestring, cyclenumber):
    ...
    if not exists(dir_name):
        ...
        rmtree(dir_name)
        print('Deleted the existing directory ' + str(cyclenumber))
...
def AddTextures():    
    ...
    for object in bpy.data.objects:
        ...
        if(object.name.startswith('Bin') or object.name.startswith('Collision')):
...
def DepthMap(scene):
    ...
    mapvalue.offset[0] = -60  #-57 for kiste_einfach bin
